...
predict.y.alpha <- function(dat, K, nK,
  ...
  sqdevmat <- predict.y.part(
    dat = dat, K = K, nK = nK, alpha1 = alpha1,
...
predict.y.cv <- function(sampID, nCal, nPred, misspecify,
  ...
  alpha.vals <- predict.y.alpha(
    dat = calibration, K = K, nK = nK,
  ...
  predVals <- predict.y(
    calidat = calibration, preddat = prediction,
  ...
  RMSEpr.result <- as.data.frame(cbind(
    sampID = sampID,
    nCal = nCal,
    nPred = nPred,
  ...
  RMSEp.result <- as.data.frame(cbind(
    sampID = sampID,
    nCal = nCal,
    nPred = nPred,
    alpha1 = alpha.vals$alpha1,
  ...
  final <- list(
    Ypred = predVals$Ypred, Ytrue = predVals$Ytrue, bias = bias,
